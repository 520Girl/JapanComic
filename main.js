            ...
            <vertical id="configPage" padding="16">
              ...
              <card margin="8" cardCornerRadius="8" cardElevation="2">
                <vertical padding="16">
                  ...
                  <checkbox id="autoRestartComic" text="自动重启漫画" checked={appConfig.autoRestartComic} />
...
function saveConfigAndContinue() {
  ...
  var config = {
    ...
    autoRestartComic: appConfig.autoRestartComic,
...
function initEvents() {
  ...
  $ui.startButton.on('click', () => {
    ...
    appConfig.autoRestartComic = $ui.autoRestartComic.isChecked()
    ...
    launcher
      ...
      .then(() => {
        ...
        if (launcher.checkResults.storage && launcher.checkResults.accessibility && launcher.checkResults.floaty) {
          ...
          // 显示权限设置界面
          launcher.fixAccessibilityService().finally(() => {
            ui.run(() => {
              $ui.startButton.attr('enabled', true)
              $ui.startButton.setText('开始运行')
    ...
    .retryNetworkRequest(
      ...
      },
      {
        ...
        onRetry: (error, retryCount, delay) => {
          toast(`激活请求失败，${delay / 1000}秒后重试 (${retryCount}/3)`)
          logger.info(`等待${delay}ms后进行第${retryCount + 1}次重试: ${error.message}`)
        },
        onError: (error, retryCount) => {
          toast('激活码验证失败，请检查网络连接或稍后重试！')
          logger.error(`激活请求失败，已达到最大重试次数(${retryCount}): ${error.message}`)
          if (callback) callback()
        },
      }
    )
    .catch((error) => {
      // 这里不需要额外处理，onError已经处理了
    })
}
  ...
  if (!launcher.checkResults.accessibility) {
    ...
    launcher
      ...
      .then((success) => {
        if (success) {
          // 无障碍服务已成功启用，继续检查其他权限
          if (launcher.checkResults.storage && launcher.checkResults.floaty) {
            // 所有权限都已获取，继续执行
            saveConfigAndContinue()
          } else {
            // 继续检查其他权限
    ...
    if (!launcher.checkResults.storage) {
      // 处理存储权限
      launcher.checkStoragePermission().then(() => {
        if (launcher.checkResults.storage) {
          showPermissionSettings()
        }
      })
    ...
    if (!launcher.checkResults.floaty) {
      // 处理悬浮窗权限
      launcher.checkFloatyPermission().then(() => {
        if (launcher.checkResults.floaty) {
          showPermissionSettings()
        }
      })
